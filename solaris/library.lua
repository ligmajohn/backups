-- Modifications by me and LynXzl
...
local GetObjects = function(url)
	return {InsertService:LoadLocalAsset(url):Clone()}
end

local NotificationFrame = GetObjects("rbxassetid://6924028278")[1]
...
local SolarisLib = {
    ...
    Settings = {
        ...
        InvisibleNotifications = false,
        CloseBind = "RightControl"
    },
    Flags = {},
    CurrentTab
}
local CurrentLoadedCfg = "DefaultConfig"



local MainUI = GetObjects("rbxassetid://7835727566")[1]
...
function SolarisLib:New(Config)
    ...
    if Config.ConfigName and type(Config.ConfigName) == "string" then
        CurrentLoadedCfg = Config.ConfigName
    end
    ...
    local TabPreset = GetObjects("rbxassetid://6958615840")[1]
    local SectionPreset = GetObjects("rbxassetid://7121846230")[1]
    local ContainerPreset = GetObjects("rbxassetid://7121886326")[1]
    ...
    local MenuBtnPreset = GetObjects("rbxassetid://7037141226")[1]
    ...
    function MusicConstructor()
        ...
        local MusicFrame, MusicPreset = GetObjects("rbxassetid://7296373622")[1], GetObjects("rbxassetid://7296615234")[1]
...
local getfunnyasset = getsynasset or getcustomasset
local function PlaySong(id, title)
    Sound:Stop()
    playing = true
    if tostring(id):sub(1, 4) == "c://" and getfunnyasset then
        Sound.SoundId = getfunnyasset(tostring(id):gsub("c://", ""))
    else
    ...
    function SettingsConstructor()
        local Settings, SettingsFrame, TabPreset, ContainerPreset, TogglePreset, BindPreset, DropdownPreset, OptionPreset = {}, GetObjects("rbxassetid://7167491516")[1], GetObjects("rbxassetid://7177524915")[1], GetObjects("rbxassetid://7203599409")[1], GetObjects("rbxassetid://7208643984")[1], GetObjects("rbxassetid://7219277948")[1], GetObjects("rbxassetid://7435055269")[1], GetObjects("rbxassetid://7435032496")[1]
        local fs = true
        ...
        general:ToggleSetting("Show Music On Launch", "Shows the music menu when you load the script.", SolarisLib.Settings.ShowMusicOnLaunch, "ShowMusicOnLaunch")
        general:ToggleSetting("Invisible Notifications", "Hides notifications if the gui isn't toggled.", SolarisLib.Settings.InvisibleNotifications, "InvisibleNotifications")
	...
	UserInputService.InputBegan:Connect(function(Input)
		if (Input.KeyCode.Name == SolarisLib.Settings.CloseBind or Input.UserInputType.Name == SolarisLib.Settings.CloseBind) and not closebindbinding then
            uitoggled = not uitoggled
            MainUI.Visible = uitoggled
            NotificationHolder.Enabled = SolarisLib.Settings.InvisibleNotifications and false or uitoggled
		end
	end)
    ...
    function SolarisLib:LoadCfg(cfg)
        local success, content = pcall(game:GetService('HttpService').JSONDecode, game:GetService('HttpService'), readfile(Config.FolderToSave .. "/configs/" .. (cfg or CurrentLoadedCfg) .. ".json"))
    ...
    function SolarisLib:SaveCfg(name)
        ...
        writefile(Config.FolderToSave .. "/configs/" .. (name or CurrentLoadedCfg) .. ".json", tostring(http:JSONEncode(content)))
    end

    function SolarisLib:SelfDestruct(name)
    ...
    function TabHolder:Tab(text)
        ...
        function SectionHold:Section(text)
            ...
            function ItemHold:Button(text,callback)
                local Holding = false
                local Button = GetObjects("rbxassetid://6937142338")[1]
            ...
            function ItemHold:Toggle(text,def,flag,callback)
                local Toggle,ToggleMain = {Value = false}, GetObjects("rbxassetid://6963155498")[1]
            ...
            function ItemHold:Slider(text,min,max,start,inc,flag,callback)
                local Slider,SliderMain = {Value = start}, GetObjects("rbxassetid://6967573727")[1]
            ...
            function ItemHold:Dropdown(text,list,def,flag,callback)
                local Dropdown,DropMain,OptionPreset = {Value = nil, Toggled = false, Options = list}, GetObjects("rbxassetid://7027964359")[1], GetObjects("rbxassetid://7021432326")[1]
            ...
            function ItemHold:MultiDropdown(text,list,def,flag,callback)
                local Dropdown,DropMain,OptionPreset = {Value = {}, Toggled = false, Options = list}, GetObjects("rbxassetid://7027964359")[1], GetObjects("rbxassetid://7021432326")[1]
                DropMain.Parent = Section
                DropMain.Btn.Title.Text = text
                DropMain.Name = text .. "element"
            ...
            function ItemHold:Colorpicker(text,preset,flag,callback)
                ...
                local ColorPicker, ColorPreset, DragPreset = {Value = preset, Toggled = false}, GetObjects("rbxassetid://7329998014")[1]
            ...
            function ItemHold:Label(text)
                local Label, LabelFrame = {}, GetObjects("rbxassetid://7032552322")[1]
		...
		function Label:RemoveLabel()
            ...
            function ItemHold:Textbox(text,disappear,callback)
                local Textbox, TextboxFrame = {}, GetObjects("rbxassetid://7147292392")[1]
            ...
            function ItemHold:Bind(text,preset,holdmode,flag,callback)
                local Bind, BindFrame = {Value, Binding = false, Holding = false}, GetObjects("rbxassetid://7126874744")[1]
