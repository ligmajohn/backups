-- Settings
local ReplicateCursor = false -- set it to false if you want cursor in the middle
local FPS = 75.1 -- the fps that replay will play on

--[[
	REPLAYABILITY VERSION 1.7.3
	(unofficial)

	UPDATE INFO:
	- Added Count Frames
	- Added Start at frame
	- Dying and respawning during a replay is now supported
	- Uses the game's default gravity instead of 196.2 always


	CREDITS:
	Wally for the UI Library
	Dong for the original script
	Plusgiant5 for updating the script

	HOW TO USE IT?:
	https://www.youtube.com/watch?v=UvaeUzKmqMc&t=70s
]]

loadstring(game:HttpGet("https://raw.githubusercontent.com/plusgiant5/Libraries/main/loadstrings/anti.lua", false))()
...
local NewAnimate = game:GetObjects("rbxassetid://9326088902")[1]
...
local SentDeadSignal = false -- So there is never 2 dead frames next to eachother
...
local StartFrame = 1 -- Frame to start reading at, changed by the Start at frame: box
...
local UIShown = true
local Mode = "Write"
local DefaultGravity = workspace.Gravity -- Usually DefaultGravity but some games change it
...
local function GetAnimationFunctions(IsSecondTry)
	local oldanimate = Character:WaitForChild("Animate", 1)
	if oldanimate then
		oldanimate:Destroy()
	end
	local n = NewAnimate:Clone()
	n.Disabled = true
	n.Parent = game.Players.LocalPlayer.Character
	getfenv().script = n
	local r,e = loadstring(n.Source)
	if not r then
		error("ERROR IN NEWANIMATE: "..e)
	else
		r()
	end
	repeat
		wait()
		--print("Waiting:",_G.RunFunction,_G.ClimbFunction)
	until _G.RunFunction and _G.ClimbFunction
...
local ChatGui = PlayerGui:WaitForChild("Chat", 3) -- Replay will be paused/aborted if chat is typing, you can change how long you for it
local ChatBox
if ChatGui then 
...
-- UI Initilializing
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/plusgiant5/Libraries/main/loadstrings/wallyuilib.lua", true))()
...
MainWindow:Dropdown("Mode", { 
	...
	if not CanChangeMode() then
		print("cant change mode now")
		return
	end
	Mode = ChosenMode
	ResetZoom()
...
MainWindow:Bind("Begin Replay", {
	...
	if Writing then 
		...
		inputconnection = UserInputService.InputBegan:Connect(function(k,v)
			if v then
				return
			end
			if k.KeyCode == Enum.KeyCode.F then
				answer("NO (f)")
			elseif k.KeyCode == Enum.KeyCode.P then
				answer("YES (p)")
			end
		end)
		SendNotification("Save Replay?", 15, answer, "NO (f)", "YES (p)")
	else 
...
MainWindow:Button("Start", function()
	...
	ReadStart = true
	SendNotification("Reading is starting in 2 seconds.", 3)
	delay(2, function()
		Reading = true
		ReadStart = false
	end)
...
MainWindow:Section("Other") -- Other Section 
MainWindow:Button("Disable Killbricks", function()
	local n = 0
	for _,v in pairs(workspace:GetDescendants()) do
		if (v.Name == "kills" or v.Name == "instakills" or v.Name == "poisons" or v.Name == "ouch") and v.Parent.CanTouch then
			v.Parent.CanTouch = false
			n = n + 1
		end
	end
	SendNotification("Disabled "..tostring(n).." killbricks.", 3)
end)
MainWindow:Button("Count Frames", function()
	local frames = #HttpService:JSONDecode(readfile(PlaceId.."\\Testing.lua").."1]")
	SendNotification("This replay has "..tostring(frames).." frames ("..tostring(math.floor(frames/60*100+.5)/100).." seconds long at constant 60 fps)", 10)
end)
MainWindow:Box("Start at frame: ", {default = "0"}, function(frame)
	StartFrame = tonumber(frame)
	Index = StartFrame
	SendNotification("Reading will now start at frame "..frame, 3)
end)
...
local function Write()
	while true do
		if not Writing or Saving then 
			...
			local t = tick()
			local StateTypeValue = Humanoid and Humanoid.Parent and Humanoid:GetState().Value
			if not SentDeadSignal then
				if not Hooked then 
					HookHumanoid() 
					Hooked = true 
				end
			...
			print(StateTypeValue)
			if StateTypeValue == 15 and not SentDeadSignal then -- Dead
				--print("w1")
				SentDeadSignal = true
			else
				if StateTypeValue and SentDeadSignal then -- make sure the humanoid exists, and SentDeadSignal isnt already false
					SentDeadSignal = false
					--print("w2")
				end
			end
		end
	...
	while true do
		if not Reading then 
			...
			if StateTypeValue ~= OldState then
				...
				if StateTypeValue == 3 then -- Jumping
					Humanoid.Jump = true
				else
					if StateTypeValue == 15 then
						-- press keys Esc+R+Enter then reset gravity
						keypress(0x1B)
						wait()
						keyrelease(0x1B)
						wait(math.random(15,27)/100)
						keypress(0x52)
						wait()
						keyrelease(0x52)
						wait(math.random(15,27)/100)
			...
			if StateTypeValue == 15 then -- Dead
				-- Wait until player fully respawns before replaying another frame
				--print("r1")
				LocalPlayer.CharacterAdded:Wait()
				while not ClimbFunction do
					RunService.Stepped:Wait()
				end
...
-- Loading checkpoint
local placefile = readfile(PlaceId.. "\\Testing.lua")
if placefile then
	local data = HttpService:JSONDecode(placefile.."1]")
	if #data > 1 then
		local lastframe = data[#data-1]
		PastLocation = TableToCFrame(lastframe[1])
		PastCameraLocation = TableToCFrame(lastframe[2])
		PastZoom = lastframe[3]
		LastMousePositionR = lastframe[8]
	end
end

SendNotification("Replayability v1.7.3 loaded!", 3)
...
--wait(.25)
--loadstring(game:HttpGet("https://raw.githubusercontent.com/plusgiant5/Libraries/main/loadstrings/animfix.lua", true))()
